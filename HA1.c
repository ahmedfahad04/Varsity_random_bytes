	...
	for(i=0;i<count;i++)
    {
    	for(j=0;j<50;j++)
    	{
    		if((strcmp(ch[i],v[j])==0))
    		{
    			...
    			(strcmp(ch[i-1],"They")!=0) && (strcmp(ch[i-1],"It")!=0) && (strcmp(ch[i-1],"My")!=0) && (strcmp(ch[i-1],"he")!=0) && (strcmp(ch[i-1],"she")!=0) && (strcmp(ch[i-1],"we")!=0) && (strcmp(ch[i-1],"you")!=0) && 
...
/*
	//this part also count verb bases on some fixed suffix like 'ed', 'es' etc. Others complex cases are not appllicable.
